    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (ImGui::Begin("Viewport", nullptr, viewport_flags))
        {
            ...
            if (viewport_size.x > 0.f && viewport_size.y > 0.f)
            {
                ...
                // Mini isometric inset (top-right).
                {
                    int inset = static_cast<int>(std::min(vw, vh) * 0.35f);
                    int margin = 10;
                    int inset_x = vx + vw - inset - margin;
                    int inset_y = vy + vh - inset - margin;
                    glViewport(inset_x, inset_y, inset, inset);
                    glScissor(inset_x, inset_y, inset, inset);
                    glClear(GL_DEPTH_BUFFER_BIT);

                    float yaw_iso = to_radians(45.0f);
                    float pitch_iso = to_radians(65.0f);
                    float dist_iso = 28.0f;
                    float cy2 = std::cos(yaw_iso);
                    float sy2 = std::sin(yaw_iso);
                    float cp2 = std::cos(pitch_iso);
                    float sp2 = std::sin(pitch_iso);
                    Vec3 eye2{dist_iso * sy2 * cp2, dist_iso * sp2, dist_iso * cy2 * cp2};
                    Mat4 view2 = look_at(eye2, Vec3{0.f, 0.f, 0.f}, Vec3{0.f, 1.f, 0.f});
                    Mat4 proj2 = perspective(55.0f, 1.0f, 0.1f, 120.0f);
                    Mat4 mvp_world2 = multiply(proj2, view2);

                    auto draw_mesh_mvp2 = [&](const GlMesh& mesh, const Mat4& mvp, const Vec3& tint, float alpha)
                    {
                        Vec3 t = apply_tone(tint);
                        glUniformMatrix4fv(u_mvp_loc, 1, GL_FALSE, mvp.m.data());
                        glUniform3f(u_tint_loc, t.x, t.y, t.z);
                    ...
                    auto draw_mesh2 = [&](const GlMesh& mesh, const Mat4& model, const Vec3& tint, float alpha)
                    {
                        Mat4 mvp = multiply(mvp_world2, model);
                        draw_mesh_mvp2(mesh, mvp, tint, alpha);
                    };

                    draw_mesh2(bottom_mesh, identity(), Vec3{1.f, 1.f, 1.f}, 1.0f);
                    draw_mesh2(floor_mesh, identity(), Vec3{1.f, 1.f, 1.f}, 1.0f);
                    draw_mesh2(walls_mesh, identity(), Vec3{1.f, 1.f, 1.f}, 1.0f);

                    if (const auto& p = game.active_piece())
                    {
                        draw_mesh2(wireframe_active ? active_edges : active_mesh, identity(), p->color, wireframe_active ? 0.95f : 0.6f);
                    }

                    if (const auto ghost = game.ghost_piece())
                    {
                        draw_mesh_mvp2(active_edges, mvp_world2, Vec3{0.7f, 0.75f, 0.8f}, 0.3f);
                    }
