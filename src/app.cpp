...
int main()
{
    ...
    auto draw_mesh = [&](const GlMesh& mesh, const Mat4& model, const Vec3& tint, float alpha)
        ...
        {
            auto ghost_vertices = build_piece_mesh(*ghost, game.well(), cell_size);
            update_mesh(active_mesh, ghost_vertices);
            glDisable(GL_CULL_FACE);
            glDepthMask(GL_FALSE);
            glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
            draw_mesh(active_mesh, identity(), ghost->color, 0.22f);
